...
int Player::moveIA(int **Mapa){
	...
	if((aux=UP(Mapa))>=1){
		...
		if(PlayerMap[PosiLinha+1][PosiCol]==PosVisitada)
		{
			PlayerMap[PosiLinha+1][PosiCol]=player;
...
int **inicializaMatriz(int** Mapa, Wumpus W){
	...
	int auxLinha = W.getPosiLinhaW();
	int auxColuna = W.getPosiColW();
